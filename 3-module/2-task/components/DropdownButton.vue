<template>
  <div class="dropdown" :class="{ show: showOptions }" v-model="value">
    <button type="button" class="button dropdown__toggle" :class="{ dropdown__toggle_icon: hasIcon } " @click="clickButtonHandle">
      <app-icon v-if="buttonIcon" :icon="buttonIcon"/>
      {{ buttonTitle }}
    </button>

    <div class="dropdown__menu" :class="{ show: showOptions }" >
      <button v-for="option in options" class="dropdown__item" :class="{ dropdown__item_icon: hasIcon }" type="button" :value="option.value" @click="selectOptionHandle(option.value)">
...
export default {
  ...
  computed: {
    buttonTitle() {
      return this.title + (this.value != null ? ` - ${this.options.find(item=> item.value === this.value).text }` : '');
    },
    hasIcon() {
